...
def play_audio(response):
    ...
    stream = audio.open(format=OUT_FORMAT, channels=OUT_CHANNELS, rate=OUT_RATE, output=True)
    try:
...
def record_and_send():
    audio = pyaudio.PyAudio()
    stream = audio.open(format=IN_FORMAT, channels=IN_CHANNELS, rate=IN_RATE, input=True, frames_per_buffer=IN_CHUNK)
